...
约束（与本地流程完全一致）:
  ...
  - 每条摘要 50-65 字（含标点，validate 缓冲 30-70）
  - 5 个板块各 3-6 条
...
_rpm_history = {}      # host -> [timestamp, ...] 最近 60s 内的请求时刻
_detected_rpm = {}     # host -> 从 429 响应里学到的 RPM 上限
...
def _post_with_retry(url, headers, data, timeout=60, max_retry=3):
    ...
    for i in range(max_retry):
        ...
        if r.status_code == 429:
            wait = _parse_429_wait(r)
            if i < max_retry - 1:
                print(f"  ⚠ 429 限流，等待 {wait:.1f}s 后重试 ({i+1}/{max_retry})")
                time.sleep(wait)
                continue
            print(f"  ⚠ 429 限流耗尽重试次数，交给上层兜底")
            return r
        return r
    return r
...
def build_section_prompt(sec, day, cfg, ctx_for_sec, errors, seen_texts=None, seed=""):
    ...
    usr_p = (
        ...
        f"• 字数按中文字符计算（含标点符号），每条目标 50–65 字，必须在 {cfg['item_char_min']}-{cfg['item_char_max']} 字之间，"
        f"不足 50 字时必须补充具体时间、地点、主体或影响扩写到 50 字以上。\n\n"
    )
    if seen_texts:
        usr_p += (
...
def build_batch_prompt(batch, day, cfg, ctx_map, seen_texts=None, errors_map=None, seed=""):
    ...
    sys_p = (
        ...
        "规则 1：每条 text 为 50–65 个汉字的客观陈述句，不评论不引申；不足 50 字必须补充真实细节扩写到 50 字以上。\n"
    ...
    for sec in batch:
        ...
        blocks.append(
            ...
            f"覆盖不同事件，每条目标 50–65 字、必须在 {cfg['item_char_min']}-{cfg['item_char_max']} 字之间。\n"
            f"该板块可用检索素材：\n{ctx_txt}\n"
        )
...
def _fill_missing_items(existing, missing, sec, day, cfg, ctx_for_sec, seen_texts,
    ...
    sys_p = (
        ...
        "规则 2：每条 text 为 50-65 个汉字的客观陈述句，source_idx 必须来自来源编号表。\n"
...
def build_meta_prompt(day, cfg, prev_type, search_ctx, errors, seed=""):
    ...
    sys_p = (
        ...
        "每条必须写具体事件标题（含主体/事件/数字/平台，如'黑神话悟空Steam销量破千万 微博热搜'），"
        "≤25字，site 只能是给定热榜站点之一；"
        "严禁泛化占位标题如'全球票房新片引热议''体育赛事今日看点''国际话题登上热搜'等。\n"
    ...
    usr_p = (
        ...
        f"今日一问选题方向：以国际趋势、公共服务、城市生活观察、科技伦理/环境/教育等"
        f"有公共讨论价值的话题为主，面向30-40岁城市读者；可结合当日新闻热点但避免娱乐八卦。"
        f"topic 要有钩子（如 ask「您怎么看待某国际趋势对普通人生活的影响？评论区聊聊」、"
        f"stance「城市『15分钟生活圈』对您来说方便吗？| 方便 | 还不完善」）。\n"
        f"严禁时政外交、军事、灾情伤亡、政策抱怨、荐股、医疗建议、性别地域对立等易翻车话题；"
        f"互动问题里不得出现『政策』『时政』『外交』『政府』『制裁』『关税』等敏感词。\n"
        f"检索素材（仅供参考选题方向）：\n"
        ...
        '    {"text": "某电影票房破X亿登顶暑期档", "site": "微博热搜"},\n'
        '    {"text": "某地推出15分钟便民生活圈试点", "site": "百度热搜"},\n'
        '    {"text": "某国际赛事中国队晋级八强", "site": "抖音热榜"},\n'
        '    {"text": "某新能源车企发布千元级车型", "site": "知乎热榜"},\n'
        '    {"text": "某城市调整地铁运营时间", "site": "今日头条"},\n'
        '    {"text": "某城市公共空间改造方案引热议", "site": "微博热搜"}\n'
        '  ]},\n'
        '  "interaction": {"title": "今日一问", "lead": "引导语≤30字",\n'
        '    "card": {"type": "ask", "topic": "结合国际趋势或城市生活观察的开放问题", "hint": "..."}, "closing": "收尾≤20字"}\n'
        "}\n"
    )
    if errors:
        usr_p += "\n上一次未通过校验：\n- " + "\n- ".join(errors)
...
def _meta_fallback(cfg, day):
    ...
    hot_texts = [
        "国产新片单日票房破2亿引发讨论", "某城市地铁新线开通首日客流创新高",
        "国际能源价格波动引发关注", "多地启动秋季开学交通保障",
        "新能源汽车9月销量数据公布", "某项城市服务新举措今日上线"
    ]
    hot_items = [
    ...
    interaction = {
        "title": "今日一问",
        "lead": "今天想和大家聊点身边或远方的公共话题。",
        "card": {"type": "ask",
                 "topic": "最近哪项城市服务新变化或国际趋势，让您觉得和日常生活关系最密切？",
                 "hint": "简单说说它对您或身边人的影响。"},
        "closing": "期待您的观察和观点。",
    }
...
def build_prompt(day, cfg, prev_type, search_ctx, errors, seed=""):
    ...
    sys_prompt = (
        ...
        "2. 每条新闻摘要严格 50–65 个汉字（含标点），用客观陈述句，不评论、不引申；宁可稍详勿过简。"
        "若某条摘要不足 50 字，必须通过补充时间、地点、主体、影响等关键细节进行扩写，"
        ...
        "7. 【今日一问·面向成年读者】选题以国际趋势、社会民生、公共议题、城市生活观察为主，"
        "兼顾 30–40 岁城市读者的表达习惯：①国际时事对生活的影响（贸易、能源、气候、科技伦理）；"
        "②社会民生与公共服务（教育、医疗、交通、养老、就业、社区治理）；"
        "③城市生活观察（通勤、住房、消费、垃圾分类、公共空间、15分钟生活圈）；"
        "④科技与环境（AI 治理、新能源、极端天气、绿色生活、环保行动）；"
        "⑤可作为调剂的轻话题仅限季节/城市/生活小技巧。"
        "严禁拿时政外交站队、军事、灾情伤亡、民生政策抱怨（油价房价社保养老医保裁员物价）、投资荐股、"
        "医疗健康建议（偏方疗效治病保健品降压降糖减肥中药处方）、点名个人是非、性别地域阶层对立等易翻车话题来提问；"
        "互动问题里不得出现『政策』『时政』『外交』『政府』『制裁』『关税』等敏感词。\n"
    ...
    user_prompt = (
        ...
        f"互动卡质量红线：①给一个让人想立刻回答的具体场景，优先结合当日国际趋势/城市生活/科技环境话题；"
        f"②给模板或格式降低参与门槛；"
        f"③topic 要有钩子（例：stance「城市『15分钟生活圈』对您方便吗？| 方便 | 还不完善」；"
        f"ask「某国际趋势对您所在行业或生活有影响吗？评论区聊聊」；"
        f"fill「我认为今年最期待的生活新变化是__，您呢？」）；④lead/closing 有温度像聊天。\n\n"
    )
    if theme:
        user_prompt += (
    ...
    user_prompt += (
        ...
        '  "quote": "原创/公版励志微语（≤30字）",\n'
        '  "sections": [\n'
        '    {"name": "国内要闻", "items": [{"text": "50-65字摘要", "source": "央视新闻"}]},   // source 仅限：新华社/人民日报/央视新闻/中国新闻网/央视网\n'
        '    {"name": "国际新闻", "items": [{"text": "50-65字摘要", "source": "新华社"}]},     // source 仅限：新华社/央视新闻/环球时报/参考消息/中国日报\n'
        '    {"name": "财经动态", "items": [{"text": "50-65字摘要", "source": "证券时报"}]},   // source 仅限：新华社/央视新闻/证券时报/第一财经/财联社\n'
        '    {"name": "科技前沿", "items": [{"text": "50-65字摘要", "source": "科技日报"}]},   // source 仅限：新华社/央视新闻/科技日报/IT之家/量子位\n'
        '    {"name": "社会民生", "items": [{"text": "50-65字摘要", "source": "澎湃新闻"}]},   // source 仅限：新华社/央视新闻/澎湃新闻/新京报/中国青年报\n'
        '    {"name": "文体资讯", "items": [{"text": "50-65字摘要", "source": "新京报"}]}     // source 仅限：新华社/央视新闻/新京报/澎湃新闻/中国青年报\n'
        "  ],\n"
