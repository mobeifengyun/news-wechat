...
def build_meta_prompt(day, cfg, prev_type, search_ctx, errors, seed=""):
    ...
    usr_p = (
        ...
        f"今日一问选题方向：以国际时事趋势、社会民生公共议题、城市生活观察、科技伦理/环境/教育等"
        f"有公共讨论价值的话题为主，面向30-40岁城市读者；可结合当日新闻热点但避免娱乐八卦。"
        f"topic 要有钩子（如 ask「您怎么看待某国际事件对普通人生活的影响？评论区聊聊」、"
        ...
        '    {"text": "某社会政策即日起正式施行", "site": "微博热搜"}\n'
        '  ]},\n'
        '  "interaction": {"title": "今日一问", "lead": "引导语≤30字",\n'
        '    "card": {"type": "ask", "topic": "结合国际/社会民生的开放问题", "hint": "..."}, "closing": "收尾≤20字"}\n'
        "}\n"
    )
    if errors:
        usr_p += "\n上一次未通过校验：\n- " + "\n- ".join(errors)
...
def _meta_fallback(cfg, day):
    ...
    interaction = {
        ...
        "card": {"type": "ask",
                 "topic": "最近哪项社会政策或国际事件，让您觉得和日常生活关系最密切？",
                 "hint": "简单说说它对您或身边人的影响。"},
        "closing": "期待您的观察和观点。",
    }
...
def build_prompt(day, cfg, prev_type, search_ctx, errors, seed=""):
    ...
    sys_prompt = (
        ...
        "③城市生活观察（通勤、住房、消费、垃圾分类、公共空间）；"
        "④科技与环境（AI 治理、新能源、极端天气、环保政策）；"
    ...
    user_prompt = (
        ...
        f"互动卡质量红线：①给一个让人想立刻回答的具体场景，优先结合当日国际/社会民生新闻；"
        f"②给模板或格式降低参与门槛；"
        f"③topic 要有钩子（例：stance「城市『15分钟生活圈』对您方便吗？| 方便 | 还不完善」；"
        f"ask「某国际事件对您所在行业或生活有影响吗？评论区聊聊」；"
        f"fill「我认为今年最值得关注的公共议题是__，您呢？」）；④lead/closing 有温度像聊天。\n\n"
    )
    if theme:
        user_prompt += (
